...
with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    # BM25 with line documents
    line_docs = store.read_line_documents()
    print('Line docs:')
    for doc in line_docs:
        print(f'  line={doc.line_number}, source={doc.source}, content={repr(doc.content[:50])}')
    
    searcher = BM25MemorySearcher(line_docs)
    bm25_hits = searcher.search('markdown memory', top_k=4)
    print(f'\nBM25 hits: {len(bm25_hits)}')
    for hit in bm25_hits:
        print(f'  score={hit.score:.3f}, source={hit.document.source}, line={hit.document.line_number}, content={repr(hit.content[:50])}')
    ...
    for result in results:
        print(f'  source_id={result.source_id}, content={repr(result.content[:50])}')
